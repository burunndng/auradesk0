...
const defaultDesktopIcons: DesktopIcon[] = [
  ...
  { id: 'desk-notes', name: 'Notes', icon: 'Feather', appId: 'notes', position: { x: 16, y: 286 }, isSelected: false },
...
function osReducer(state: OSState, action: OSAction): OSState {
  switch (action.type) {
    ...
    case 'MAXIMIZE_WINDOW': {
      const vw = action.viewport?.width ?? 1024;
      const vh = action.viewport?.height ?? 768;
      return {
        ...state,
...
export const OSProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    localStorage.setItem('serk3tos_desktop_icons', JSON.stringify(state.desktopIcons));
  }, [state.desktopIcons]);
...
export const useWindows = () => {
  ...
  return {
    ...
    openWindow: useCallback((appId: string, title?: string) => dispatch({ type: 'OPEN_WINDOW', appId, title, viewport: { width: window.innerWidth, height: window.innerHeight } }), [dispatch]),
    ...
    maximizeWindow: useCallback((windowId: string) => dispatch({ type: 'MAXIMIZE_WINDOW', windowId, viewport: { width: window.innerWidth, height: window.innerHeight } }), [dispatch]),

...
function handleMenuAction(action: string, _state: unknown, dispatch: Dispatch<import('@/types').OSAction>) {
  const [cmd, ...args] = action.split(':');
  switch (cmd) {
    case 'OPEN_APP': {
      if (args[0]) dispatch({ type: 'OPEN_WINDOW', appId: args[0], viewport: { width: window.innerWidth, height: window.innerHeight } });
      break;
    }
    case 'NEW_FOLDER':
    case 'NEW_DOCUMENT':
